...
export const
    memberValidatorMiddleware =
    {
        ...
        emailChange: async (req: Request, res: Response, next: NextFunction) =>
        {
            try
            {
                req.validated = await memberValidator.emailChange.validate(req.body);
                next();
            }
            catch(error: any)
            {
